...
class SearchPapersInput(BaseModel):
    """Input schema to search papers with the CORE API."""
    query: str = Field(description="The query to search for on the selected archive.")
    max_papers: int = Field(
        description="The maximum number of papers to return. It defaults to 1, but you can increase it up to 10 in case you need to perform a more comprehensive search.",
        default=1,
        ge=1,
        le=10
    )
...
class DecisionMakingOutput(BaseModel):
    """Output schema of the decision making node."""
...
class JudgeOutput(BaseModel):
    """Output schema of the judge node."""
